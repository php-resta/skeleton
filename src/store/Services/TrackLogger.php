...
class TrackLogger extends ApplicationProvider
{
    ...
    public function handle($callback)
    {
        ...
        if(!isset($filterResult) || (isset($filterResult) && is_array($filterResult) && !in_array(false,$filterResult))){
            ...
            $httpMethod = isset($this->output['httpMethod']) ? $this->output['httpMethod'] : null;

            echo PHP_EOL;
            echo 'Http Method: '.$httpMethod.'';

            echo PHP_EOL;
            echo 'Get Data: '.json_encode(isset($this->output['getData']['params']) ? $this->output['getData']['params'] : []);

            echo PHP_EOL;
            echo 'Post Data: '.json_encode(isset($this->output['postData']) ? $this->output['postData'] : []);

            echo PHP_EOL;
            echo 'Put Data: '.json_encode(isset($this->output['putData']) ? $this->output['putData'] : []);

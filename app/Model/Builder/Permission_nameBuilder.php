...
use App\Munch\Api\V1\Model\Permission_name;
use Resta\Contracts\ClientContract;
use App\Munch\Api\V1\Model\Contract\Permission_nameContract;
use App\Munch\Api\V1\Model\Builder\Assistant\Builder;
use Illuminate\Database\ConnectionInterface;

class Permission_nameBuilder implements Permission_nameContract
{
    use Builder;
     ...
     * @var Permission_name
     */
    protected static $model = Permission_name::class;

    /**
     * get all data for Permission_name model
     *
     * @return mixed
     */
    public function all()
    {
        //model all data
        return Permission_name::all();
    }

    /**
     * create Permission_name model
     *
    ...
    public function create(ClientContract $client)
    {
        ...
        try{
            return $this->UniqueEntityForIsDeleted($client,function() use($client){
                ...
                exception('database_'.$exception->getCode(),['key'=>'permission_name_code:'.$client->input('permission_name_code')])
     ...
     *
     * @param $data
     * @return Permission_name|void
     */
    public function createIfNotExist($data)
    {
        //create Permission_name
        try{
            $data = static::$model::where('permission_name_code',$data)->first();

            if(is_null($data)){
                return static::$model::create(['permission_name_code'=>$data]);
            }
    ...
    /**
     * get Permission_name find method for model
     *
    ...
    public function get()
    {
     ...
     public function update(ClientContract $client,$createIfNotExist=false)
     {
        ...
        if(!isset($data)){
            $data = static::$model::where('permission_name_code',$client->input('permission_name_code'))->first();
        }

        nullExceptionPointer($data,'permission_name_code:'.$client->input('permission_name_code'));
        ...
        try{
            $data->save();
            return static::$model::where('permission_name_code',$client->input('permission_name_code'))->first();
        }catch (Exception $e){
                 exception('database_'.$e->getCode(),['key'=>$client->input('permission_name_code')])
                     ->invalidArgument($e->getMessage());
        }
     }

     /**
      * update array for Permission_name model
      *
      * @param ClientContract $client
     ...
     {
        $clientArray = clientArray($client,'permission_name_code');

...
use App\Munch\Api\V1\Model\Tableevent;
use Resta\Contracts\ClientContract;
use App\Munch\Api\V1\Model\Contract\TableeventContract;
use App\Munch\Api\V1\Model\Builder\Assistant\Builder;
use Illuminate\Database\ConnectionInterface;

class TableeventBuilder implements TableeventContract
{
    use Builder;
     ...
     * @var Tableevent
     */
    protected static $model = Tableevent::class;

    /**
     * get all data for Tableevent model
     *
     * @return mixed
     */
    public function all()
    {
        //model all data
        return Tableevent::all();
    }

    /**
     * create Tableevent model
     *
    ...
    public function create(ClientContract $client)
    {
        ...
        try{
            return $this->UniqueEntityForIsDeleted($client,function() use($client){
                ...
                exception('database_'.$exception->getCode(),['key'=>'tableevent_code:'.$client->input('tableevent_code')])
     ...
     *
     * @param $data
     * @return Tableevent|void
     */
    public function createIfNotExist($data)
    {
        //create Tableevent
        try{
            $data = static::$model::where('tableevent_code',$data)->first();

            if(is_null($data)){
                return static::$model::create(['tableevent_code'=>$data]);
            }
    ...
    /**
     * get Tableevent find method for model
     *
    ...
    public function get()
    {
     ...
     public function update(ClientContract $client,$createIfNotExist=false)
     {
        ...
        if(!isset($data)){
            $data = static::$model::where('tableevent_code',$client->input('tableevent_code'))->first();
        }

        nullExceptionPointer($data,'tableevent_code:'.$client->input('tableevent_code'));
        ...
        try{
            $data->save();
            return static::$model::where('tableevent_code',$client->input('tableevent_code'))->first();
        }catch (Exception $e){
                 exception('database_'.$e->getCode(),['key'=>$client->input('tableevent_code')])
                     ->invalidArgument($e->getMessage());
        }
     }

     /**
      * update array for Tableevent model
      *
      * @param ClientContract $client
     ...
     {
        $clientArray = clientArray($client,'tableevent_code');

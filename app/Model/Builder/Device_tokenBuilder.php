...
use App\Munch\Api\V1\Model\Device_token;
use Resta\Contracts\ClientContract;
use App\Munch\Api\V1\Model\Contract\Device_tokenContract;
use App\Munch\Api\V1\Model\Builder\Assistant\Builder;
use Illuminate\Database\ConnectionInterface;

class Device_tokenBuilder implements Device_tokenContract
{
    use Builder;
     ...
     * @var Device_token
     */
    protected static $model = Device_token::class;

    /**
     * get all data for Device_token model
     *
     * @return mixed
     */
    public function all()
    {
        //model all data
        return Device_token::all();
    }

    /**
     * create Device_token model
     *
    ...
    public function create(ClientContract $client)
    {
        ...
        try{
            return $this->UniqueEntityForIsDeleted($client,function() use($client){
                ...
                exception('database_'.$exception->getCode(),['key'=>'device_token_code:'.$client->input('device_token_code')])
     ...
     *
     * @param $data
     * @return Device_token|void
     */
    public function createIfNotExist($data)
    {
        //create Device_token
        try{
            $data = static::$model::where('device_token_code',$data)->first();

            if(is_null($data)){
                return static::$model::create(['device_token_code'=>$data]);
            }
    ...
    /**
     * get Device_token find method for model
     *
    ...
    public function get()
    {
     ...
     public function update(ClientContract $client,$createIfNotExist=false)
     {
        ...
        if(!isset($data)){
            $data = static::$model::where('device_token_code',$client->input('device_token_code'))->first();
        }

        nullExceptionPointer($data,'device_token_code:'.$client->input('device_token_code'));
        ...
        try{
            $data->save();
            return static::$model::where('device_token_code',$client->input('device_token_code'))->first();
        }catch (Exception $e){
                 exception('database_'.$e->getCode(),['key'=>$client->input('device_token_code')])
                     ->invalidArgument($e->getMessage());
        }
     }

     /**
      * update array for Device_token model
      *
      * @param ClientContract $client
     ...
     {
        $clientArray = clientArray($client,'device_token_code');
